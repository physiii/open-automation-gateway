...
io.on('connection', (socket) => {
    ...
    socket.on('searchForNetworkCameras', async () => {
        console.log(TAG, "Searching for network cameras.");
    
        try {
            const result = await DeviceUtils.searchForNetworkCameras();
            console.log('Discovered network cameras:', result);
            socket.emit('discoveredCameras', result);
        } catch (error) {
            console.error("Error searching for network cameras:", error);
        }
    });

    socket.on('addCamera', async (camera) => {
        console.log("Camera to add:", camera);
        let { ip, username, password } = camera;

        try {
            const result = await DeviceUtils.createNetworkCameraService(ip, username, password);
            console.log('camera was added:', result);
            socket.emit('addCamera', result);
        } catch (error) {
            console.error("Error adding network cameras", error);
        }
    });
	...
	socket.on('removeDevice', (data) => {
        console.log(TAG, "Removing device:", data);
		removeDevice(data);
	});
