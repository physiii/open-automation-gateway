...
##################################################################################################################
# Required Imports to run motion detection
...
##################################################################################################################
#Start motion Detection program and build Device list array from MongoDB
...
if connection is not None:
  ...
  for device in devices:
    ...
    vs.append(VideoStream(src=device_obj["dev"],usePiCamera=0,resolution=[800,600],framerate=20).start());
    print("loaded " + device_obj['dev'])
    sys.stdout.flush()
    i=i+1

#kcw = KeyClipWriter(bufSize=64)
#consecFrames = 0
##################################################################################################################
...
# Begin Camera Loop for Video Capture
while True:
    ...
    #Build Temp File Directory if Directory doesn't Already Exist
    ...
    #Converts image from RGB color to grayscale
	gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    #Blurs the contour edging to identify pixel intensity more evenly
